...
#Variabelen voor muren-doorlaatbaarheid
vijanden_muren_starttijd = 0  # Houdt steeds bij wanneer de fase start
vijanden_muren_actief = False  # Of de vijanden door de muren kunnen of niet (boolean)
vijanden_muren_duur = 5       # Hoe lang het duurt (in sec)
...
while running:
    ...
    # Timer-logica
    huidige_tijd = pygame.time.get_ticks() / 1000 # Tijd van miliseconden naar seconden (/1000)
    cyclustijd = huidige_tijd % 35  # Bepaal in welke fase we zitten (0-35 seconden cyclus)

    if 30 <= cyclustijd < 35:  # Activeer muur-doorlaatbaarheid na 30 seconden
        if not vijand_muurfase_actief:
            vijand_muurfase_actief = True
            vijand_muurfase_starttijd = huidige_tijd
            for vijand in [leeuw, olifant, neushoorn, gier]:
                vijand.kan_door_muren = True
            print("Vijanden kunnen door muren! (5seconden)")

        else:  # Normale fase, deactiveer na 5 seconden
            if vijand_muurfase_actief:
                vijand_muurfase_actief = False
                for vijand in [leeuw, olifant, neushoorn, gier]:
                    vijand.kan_door_muren = False
                print("Vijanden kunnen niet meer door muren (30 seconden).")
    ...
    def volg_pad(self):
        if self.doel_index < len(self.pad):
            ...
            if afstand < self.snelheid:
               ...
               if self.kan_door_muren: #kan.door_muren is "true"
                    self.x += self.snelheid * dx / afstand
                    self.y += self.snelheid * dy / afstand

               else:  # Normale beweging (met muur-check)
                    nieuw = pygame.Vector2(self.snelheid * dx / afstand, self.snelheid * dy / afstand)
                    controlewaarden = [
                        (nieuw.x + 18, nieuw.y + 18),  # (x, y) als tuple
                        (nieuw.x + 18, nieuw.y - 18),
                        (nieuw.x - 18, nieuw.y + 18),
                        (nieuw.x - 18, nieuw.y - 18)
                    ]
                    if all(maze_checker.is_valid(round(x), round(y)) for x, y in controlewaarden):
                        self.rect.x += nieuw.x  # Pas positie aan als alles geldig is
                        self.rect.y += nieuw.y
